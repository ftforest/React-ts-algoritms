...
export const editPost = createAsyncThunk('posts/editPost', async (updatedPost:any) => {
...
const postsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deletePost.fulfilled, (state:any, action) => {
      state.list = state.list.filter((post:any) => post.id !== action.payload.id);
    });
    builder.addCase(deletePost.rejected, (state:any, action) => {
      state.error = action.payload;
    });
    ...
    builder.addCase(editPost.rejected, (state:any, action) => {
      state.error = action.payload;
    });

    builder.addCase(createNewPost.fulfilled, (state:any, action) => {
      state.list = [...state.list, action.payload];
    });
    builder.addCase(createNewPost.rejected, (state:any, action) => {
      state.error = action.payload;
    });
  }
});
...
export const selectPostsData = ((state:any) => state.posts);